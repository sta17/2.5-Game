...
func _fill_missing_slots():
	if not create_slot_if_needed:
		if slots.size() == 0:
			_load_slots()
		else:
			var extras:int = slots.size()
			for i in (slot_amount):
				var newIndex: int = i + extras
				if newIndex == slot_amount:
					return
				_add_slot(newIndex, false)
			slot_amount = slots.size()
